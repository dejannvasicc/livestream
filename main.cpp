...
int main(int argc, const char *argv[])
{
    ...
    // FROM CODE
    decoder = new MESAI::FFmpegDecoder("C:/tmp/bunny.h264");
    decoder->intialize();
    decoder->setOnframeCallbackFunction(onFrame);
    encoder = new MESAI::FFmpegH264Encoder();
    encoder->SetupVideo("C:/tmp/dunny.avi", decoder->width, decoder->height, decoder->frameRate, decoder->GOP, decoder->bitrate);
